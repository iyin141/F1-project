...
from api.models import TaskRecord, DriverLapAnalysis
...
def paginate_laps(self, task_key: str, year: int, round_number: int, session: str, page: int = 1, page_size: int = 50):
    """
    Paginate and serialize lap data for a session.
    Task key: paginate_laps:{year}:{round}:{session}:{page}
    
    Publishes full serialized paginated lap payload via pub/sub and caches for non-blocking responses.
    """
    logger.info(
        "event=celery_start task=paginate_laps task_key=%s year=%s round=%s session=%s page=%d page_size=%d",
    ...
    try:
        # Step 1: Fetch paginated lap data from DB
        ...
        cache_key = f"paginate_laps:{year}:{round_number}:{session}:{page}"
        ...
        logger.info(
            "event=celery_success task=paginate_laps task_key=%s year=%s round=%s session=%s page=%d records=%d",
