...
def paginate_telemetry(self, task_key: str, year: int, round_number: int, session: str, page: int = 1, page_size: int = 50):
    """
    Paginate and serialize telemetry data for a session.
    Task key: paginate_telemetry:{year}:{round}:{session}:{page}
    
    Publishes full serialized paginated telemetry payload via pub/sub and caches for non-blocking responses.
    """
    logger.info(
        "event=celery_start task=paginate_telemetry task_key=%s year=%s round=%s session=%s page=%d page_size=%d",
    ...
    try:
        # Step 1: Fetch paginated telemetry data from DB
        ...
        cache_key = f"paginate_telemetry:{year}:{round_number}:{session}:{page}"
        worker_utils.handle_result(
            task_key=task_key,
            data_type="paginate_telemetry",
        ...
        logger.info(
            "event=celery_success task=paginate_telemetry task_key=%s year=%s round=%s session=%s page=%d records=%d",
