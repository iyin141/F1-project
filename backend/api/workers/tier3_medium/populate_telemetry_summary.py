...
from api.session.serializers import TelemetrySummaryResponseSerializer
import traceback
...
def populate_telemetry_summary(self, task_key: str, year: int, round_number: int, session_type: str, **kwargs):
    ...
    try:
        ...
        logger.info(
            "event=celery_success task=populate_telemetry_summary task_key=%s year=%s round=%s session=%s",
        ...
        logger.exception("event=celery_failed task=populate_telemetry_summary task_key=%s year=%s round=%s session=%s", task_key, year, round_number, session_type)
        raise
    finally:
