...
class DriverCareerAPIViewTests(TestCase):
    ...
    def test_career_valid_known_driver(self, mock_service_class):
        """Test retrieving career for valid known driver (VER)."""
        mock_service = Mock()
        mock_service_class.return_value = mock_service
        mock_service.get_driver_career.return_value = {
            "driver_code": "VER",
        ...
        self.assertEqual(len(response.data["career"]), 1)

    @patch('api.driver_views.DriverCareerService')
    def test_career_valid_unknown_driver(self, mock_service_class):
        """Test retrieving career for unknown driver returns can_proceed=false."""
        mock_service = Mock()
        mock_service_class.return_value = mock_service
        mock_service.get_driver_career.return_value = {"driver_code": "ZZZ", "driver_name": None, "nationality": None, "career": [], "career_totals": {"championships": 0, "wins": 0, "podiums": 0, "poles": 0, "fastest_laps": 0, "races_entered": 0, "dnfs": 0, "total_points": 0}}
        
        request = self.factory.get("/api/drivers/ZZZ/career/")
        response = self.view(request, driver_code="ZZZ")
        
        self.assertEqual(response.status_code, 200)
        self.assertFalse(response.data["readiness"]["can_proceed"])
        self.assertEqual(len(response.data["career"]), 0)
    ...
    def test_career_invalid_code_too_short(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertFalse(response.data["readiness"]["can_proceed"])
        self.assertIn("Invalid driver code", response.data["readiness"]["message"])
    ...
    @patch('api.driver_views.DriverCareerService')
    def test_career_case_insensitive_driver_code(self, mock_service_class):
        """Test that driver code is converted to uppercase."""
        mock_service = Mock()
        mock_service_class.return_value = mock_service
        mock_service.get_driver_career.return_value = {"driver_code": "HAM", "driver_name": "Lewis Hamilton", "nationality": "British", "career": [{"year": 2023, "constructor": "MER", "position": 2, "points": 470, "wins": 5, "podiums": 14, "poles": 4, "fastest_laps": 3, "races_entered": 22, "dnfs": 0}], "career_totals": {"championships": 7, "wins": 103, "podiums": 190, "poles": 88, "fastest_laps": 60, "races_entered": 320, "dnfs": 20, "total_points": 4500}}
        
        request = self.factory.get("/api/drivers/ham/career/")
        response = self.view(request, driver_code="ham")
        
        self.assertEqual(response.status_code, 200)
        mock_service.get_driver_career.assert_called_once_with("HAM")

    @patch('api.driver_views.DriverCareerService')
    def test_career_service_exception_returns_500(self, mock_service_class):
        """Test that unexpected service exception returns 500."""
        mock_service = Mock()
        mock_service_class.return_value = mock_service
        mock_service.get_driver_career.side_effect = RuntimeError("Unexpected database error")
    ...
    @patch('api.driver_views.DriverCareerService')
    def test_career_multiple_seasons(self, mock_service_class):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTrue(response.data["readiness"]["can_proceed"])
        self.assertEqual(len(response.data["career"]), 2)

    @patch('api.driver_views.DriverCareerService')
    def test_career_null_fields_handled(self, mock_service_class):
        """Test that null fields are handled gracefully."""
        mock_service = Mock()
        mock_service_class.return_value = mock_service
        mock_service.get_driver_career.return_value = {"driver_code": "RAI", "driver_name": None, "nationality": None, "career": [], "career_totals": {"championships": 0, "wins": 0, "podiums": 0, "poles": 0, "fastest_laps": 0, "races_entered": 0, "dnfs": 0, "total_points": 0}}
        
        request = self.factory.get("/api/drivers/RAI/career/")
        response = self.view(request, driver_code="RAI")
        
        self.assertEqual(response.status_code, 200)
        self.assertIsNone(response.data["driver_name"])
...
class DriverSeasonAPIViewTests(TestCase):
    ...
    @patch('api.driver_views.DriverCareerService')
    def test_season_valid_with_races(self, mock_service_class):
        """Test retrieving valid season with race data."""
        mock_service = Mock()
        mock_service_class.return_value = mock_service
        mock_service.get_driver_season.return_value = {
            "driver_code": "VER",
            "driver_name": "Max Verstappen",
            "year": 2023,
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTrue(response.data["readiness"]["can_proceed"])
        self.assertEqual(len(response.data["races"]), 1)

    @patch('api.driver_views.DriverCareerService')
    def test_season_valid_driver_no_data(self, mock_service_class):
        """Test retrieving season with no race data."""
        mock_service = Mock()
        mock_service_class.return_value = mock_service
        mock_service.get_driver_season.return_value = {"driver_code": "VER", "driver_name": None, "year": 1950, "constructor": None, "final_position": None, "final_points": None, "races": []}
        ...
        self.assertEqual(response.status_code, 200)
        self.assertFalse(response.data["readiness"]["can_proceed"])
        self.assertEqual(len(response.data["races"]), 0)
    ...
    def test_season_invalid_year_too_low(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertFalse(response.data["readiness"]["can_proceed"])
        self.assertIn("Invalid year", response.data["readiness"]["message"])
    ...
    @patch('api.driver_views.DriverCareerService')
    def test_season_year_boundary_1950(self, mock_service_class):
        """Test that year=1950 is valid (boundary condition)."""
        mock_service = Mock()
        mock_service_class.return_value = mock_service
        mock_service.get_driver_season.return_value = {"driver_code": "ASM", "driver_name": "Giuseppe Farina", "year": 1950, "constructor": "Alfa Romeo", "final_position": 2, "final_points": 30, "races": []}
    ...
    @patch('api.driver_views.DriverCareerService')
    def test_season_case_insensitive_driver_code(self, mock_service_class):
        """Test that driver code is uppercase in season endpoint."""
        mock_service = Mock()
        mock_service_class.return_value = mock_service
        mock_service.get_driver_season.return_value = {"driver_code": "HAM", "driver_name": "Lewis Hamilton", "year": 2023, "constructor": "Mercedes", "final_position": 2, "final_points": 470, "races": [{"year": 2023, "round": 1, "race_name": "Bahrain", "location": "Sakhir", "race_date": "2023-03-05", "grid_position": 1, "finish_position": 2, "points": 18, "status": "Finished", "fastest_lap": False, "laps_completed": 57, "qualifying_position": None, "qualifying_time": None}]}
        
        request = self.factory.get("/api/drivers/ham/2023/")
        response = self.view(request, driver_code="ham", year=2023)
        
        self.assertEqual(response.status_code, 200)
        mock_service.get_driver_season.assert_called_once_with("HAM", 2023)

    @patch('api.driver_views.DriverCareerService')
    def test_season_service_exception_returns_500(self, mock_service_class):
        """Test that unexpected service exception returns 500."""
        mock_service = Mock()
        mock_service_class.return_value = mock_service
        mock_service.get_driver_season.side_effect = RuntimeError("Database connection error")
    ...
    @patch('api.driver_views.DriverCareerService')
    def test_season_multiple_races(self, mock_service_class):
        ...
        mock_service.get_driver_season.return_value = {"driver_code": "VER", "driver_name": "Max Verstappen", "year": 2023, "constructor": "Red Bull Racing", "final_position": 1, "final_points": 575, "races": races}
        
        request = self.factory.get("/api/drivers/VER/2023/")
        response = self.view(request, driver_code="VER", year=2023)
        
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.data["races"]), 5)
        self.assertTrue(response.data["readiness"]["can_proceed"])

    @patch('api.driver_views.DriverCareerService')
    def test_season_null_fields_handled(self, mock_service_class):
        """Test that null fields in season response are handled."""
        mock_service = Mock()
        mock_service_class.return_value = mock_service
        mock_service.get_driver_season.return_value = {"driver_code": "TST", "driver_name": None, "year": 2023, "constructor": None, "final_position": None, "final_points": None, "races": []}
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIsNone(response.data["driver_name"])
        self.assertIsNone(response.data["constructor"])
...
class DriverEndpointReadinessTests(TestCase):
    ...
    @patch('api.driver_views.DriverCareerService')
    def test_career_readiness_success_structure(self, mock_service_class):
        """Verify readiness object structure when career data succeeds."""
        mock_service = Mock()
        mock_service_class.return_value = mock_service
        mock_service.get_driver_career.return_value = {"driver_code": "VER", "driver_name": "Max", "nationality": "Dutch", "career": [{"year": 2023, "constructor": "RB", "position": 1, "points": 575, "wins": 19, "podiums": 21, "poles": 7, "fastest_laps": 4, "races_entered": 22, "dnfs": 0}], "career_totals": {"championships": 1, "wins": 19, "podiums": 21, "poles": 7, "fastest_laps": 4, "races_entered": 22, "dnfs": 0, "total_points": 575}}
    ...
    @patch('api.driver_views.DriverCareerService')
    def test_season_readiness_success_structure(self, mock_service_class):
        """Verify readiness object structure when season data succeeds."""
        mock_service = Mock()
        mock_service_class.return_value = mock_service
        mock_service.get_driver_season.return_value = {"driver_code": "HAM", "driver_name": "Lewis", "year": 2023, "constructor": "Mercedes", "final_position": 2, "final_points": 470, "races": [{"year": 2023, "round": 1, "race_name": "Bahrain", "location": "Sakhir", "race_date": "2023-03-05", "grid_position": 1, "finish_position": 2, "points": 18, "status": "Finished", "fastest_lap": False, "laps_completed": 57, "qualifying_position": 1, "qualifying_time": "1:32.000"}]}
